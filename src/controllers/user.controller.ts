...
import { RefreshTokenType } from "../types/jwt.types";
...
const refreshAccessToken = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const decodedToken = decoded as RefreshTokenType;
...
const changeCurrentPassword = asyncHandler(async (req, res) => {
  ...
  const isPasswordCorrect = await typedUser.isPasswordCorrect(oldPassword);
...
const updateUserAvatar = asyncHandler(async (req, res) => {
  ...
  const userTyped = user as UserDoc;

  const PublicID = userTyped.avatar
    .split("/")
    [userTyped.avatar.split("/").length - 1].split(".")[0];
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  ...
  if (!userTyped.coverImage) throw new Error("coverImage required");

  const PublicID = userTyped.coverImage
    .split("/")
    [userTyped.coverImage.split("/").length - 1].split(".")[0];
  cloudinary.uploader.destroy(PublicID).then(console.log);
